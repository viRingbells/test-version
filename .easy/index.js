...
var range_pattern = ["~+", "\\.{2,}"];
var range_regexp = new RegExp("^(.*?\\d+.*?)(?:(?:" + range_pattern.join(")|(?:") + "))+(.*?\\d+.*?)$");
function test_range(version) {
    debug('TestVersion: test range of ' + version);
    var matches = version.match(range_regexp);
    if (!matches) {
...
function test_desc(version) {
    ...
    var matches = version.match(desc_regexp);
    if (!matches) {
